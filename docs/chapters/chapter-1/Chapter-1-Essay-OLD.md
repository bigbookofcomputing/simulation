...
1. **Initialize** the current state $\mathbf{s}_0$ and choose an inverse temperature $\beta$.
2. For each iteration $t = 0,1,2,\dots$:
   1. **Propose a candidate** $\mathbf{s}'$ from $g(\mathbf{s}_t \to \cdot)$.
   2. **Compute** the acceptance ratio
      $$
      A(\mathbf{s}_t, \mathbf{s}') = \frac{P(\mathbf{s}')\, g(\mathbf{s}' \to \mathbf{s}_t)}{P(\mathbf{s}_t)\, g(\mathbf{s}_t \to \mathbf{s}')}.
      $$
   3. **Accept or reject**: draw $u \sim \mathrm{Uniform}(0,1)$. If $u \le \min(1, A)$, set $\mathbf{s}_{t+1} = \mathbf{s}'$; otherwise set $\mathbf{s}_{t+1} = \mathbf{s}_t$.
3. **Iterate** to generate a sequence of states. After an initial “burn‑in” period the distribution of $\mathbf{s}_t$ approaches $P$.
...
To illustrate how the Metropolis algorithm works in practice, let us consider a simple yet instructive example: a single particle moving in a one‑dimensional **double‑well potential**. This toy model captures the essential difficulty of sampling multimodal distributions and highlights how temperature affects exploration.
...
1. **Initialize** the particle at some position $x_0$, typically in one of the wells.
2. For each iteration $t$:
   - **Propose** a trial move by adding a small random displacement: $x' = x_t + \delta$, where $\delta$ is drawn from a symmetric distribution (e.g., uniform in $[-\Delta, \Delta]$ or Gaussian with zero mean). The parameter $\Delta$ controls the step size.
   - **Compute** the energy difference $\Delta V = V(x') - V(x_t)$.
   - **Accept or reject** according to the Metropolis rule:
     ...
     where $u \sim \mathrm{Uniform}(0,1)$. In words: always accept moves that lower the potential, and accept uphill moves with probability $\exp(-\beta \Delta V)$.
3. **Record** the current position $x_t$ after a suitable burn‑in period to build up a histogram of visited positions.
